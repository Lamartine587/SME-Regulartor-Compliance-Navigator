...
async def get_profile(
    ...
    # Combine the database profile with the user's phone info
    return {
        "id": profile.id,
        ...
        "phone": current_user.phone, # From User table
        "is_phone_verified": current_user.is_phone_verified # From User table
    }
...
async def update_profile(
    ...
    # 1. Update Phone in the USER table if it changed
    if payload.phone and payload.phone != current_user.phone:
        ...
        current_user.is_phone_verified = False # Number changed, so reset verification

    # 2. Update BusinessProfile fields
    update_data = payload.dict(exclude_unset=True)
    for key, value in update_data.items():
        if hasattr(profile, key) and key != "phone": # phone is handled above
